...
-- ============================================================
-- 4. TRANSACTIONS — audit trail for charges + credit usage
-- ============================================================
CREATE TABLE IF NOT EXISTS transactions (
    ...
    amount_cents INTEGER NOT NULL,      -- positive = charge to user, negative = refund
    credits_delta INTEGER DEFAULT 0,    -- +5 for pack purchase, -1 for credit used
    stripe_payment_intent_id TEXT,
    description TEXT,
    created_at TIMESTAMPTZ DEFAULT now()
);
...
-- Add credits column to profiles (safe to run if column already exists)
DO $$ BEGIN
